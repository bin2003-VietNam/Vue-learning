<template>
    <div>
        <table class="table-fixed border-spacing-2 border-separate">
            <thead>
                <th class="text-left px-6 py-2">Name</th>
                <th class="text-left px-6 py-2">Email</th>
                <th class="text-left px-6 py-2">Status</th>
            </thead>
            <tbody>
                <TeamMember v-for="member in team.members" v-bind:key="member" :name='member.name' :email='member.email'
                    :status="member.status" />
            </tbody>
        </table>
        <p class="text-right text" v-show="!team.spotsRemaining">There are no remaining
            team spots. Upgrade to add more.</p>
    </div>
</template>
<script setup>
import TeamMember from "@/components/Teams/TeamMember.vue";

import { useTeamStore } from '@/store/TeamStore';

let team = useTeamStore()
</script>
